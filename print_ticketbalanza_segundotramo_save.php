<?php
  ...
  // Ruta logo
    ...
    $ruta_images = substr($ruta_images, 0, strpos($ruta_images, 'print_ticketbalanza_segundotramo_save.php')).'images/';
...
// return;
  ...
  // Verificando la ruta donde se almacenarán los archivos
    $carpeta_main = 'files/tickets_balanza/'.$cod_lote.'/SALIDA AUM/';

    // Validar si la carpeta existe
      if (!is_dir($carpeta_main)) {
        // Crear la carpeta si no existe, con permisos de lectura y escritura
          if (mkdir($carpeta_main, 0777, true)) {
            echo "Carpeta '$carpeta_main' creada exitosamente.<br>";
          }
          else{
            die("Error al crear la carpeta '$carpeta_main'.");
          }
      }
      else{
        echo "La carpeta '$carpeta_main' ya existe.<br>";
      }

  // Identificando el nombre del archivo
    $nombre_archivo = 'SALIDA AUM - '.$cod_lote.((strlen($lote_ticket_orden) == 0) ? '' : ' - PARTE '.$lote_ticket_orden);


  // Especificar la ruta donde se guardará el PDF
    $carpeta_main .= $nombre_archivo.'.pdf';

  // Guardar el archivo PDF en la ruta especificada
    file_put_contents($carpeta_main, $pdfOutput);

    echo 'Ticket generado: '.$carpeta_main;
