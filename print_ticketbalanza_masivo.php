<?php
  ...
  // Ruta logo
    ...
    $ruta_main = substr($ruta_images, 0, strpos($ruta_images, 'print_ticketbalanza_masivo.php'));
    $ruta_images = substr($ruta_images, 0, strpos($ruta_images, 'print_ticketbalanza_masivo.php')).'images/';
  ...
  // Obteniendo datos de cabecera de la recepción
    ...
    $q_balanza = "SELECT Id,
                         ...
                         placa_1,
                         placa_2,
                         empresatransporte_documento,
                         empresatransporte_razonsocial,
                         tipo_vehiculo,
                         conductor_documento,
                         conductor_nombres,
                         zona_origen,
                         ...
                         peso_neto
                    FROM print_ticket
                   WHERE Id BETWEEN 2501 AND 3000
                   /*WHERE Id IN (2565, 2567)*/
                  ORDER BY Id
                  /*LIMIT 5*/";
    ...
    if ($res_balanza = mysqli_query($enlace, $q_balanza)){
      if (mysqli_num_rows($res_balanza) > 0) {
        while($row_balanza = mysqli_fetch_array($res_balanza)){
          $Id = $row_balanza["Id"];
          $ticket_balanza = $row_balanza["num_ticket"];
          $lote_ticket_orden = $row_balanza["orden_ticket"];
          $cod_lote = $row_balanza["cod_lote"];
          $placa_1 = $row_balanza["placa_1"];
          // $placa_2 = $row_balanza["placa_2"];
          $transportista_documento = $row_balanza["empresatransporte_documento"];
          $transportista_razonsocial = $row_balanza["empresatransporte_razonsocial"];
          $tipo_vehiculo = $row_balanza["tipo_vehiculo"];
          ...
          $producto = $row_balanza["producto"];
          // $tipo_mineral = $row_balanza["TIPO_MATERIAL"];
          $observacion = $row_balanza["observaciion"];
          $guia_remitente = $row_balanza["guia_remitente"];
          $guia_transportista = $row_balanza["guia_transportista"];

          $peso_inicial = $row_balanza["peso_inicial"];
          $peso_inicial = str_replace(',', '', $peso_inicial);
          $pesoinicial_fechahora = $row_balanza["peso_inicial_fecha"];
          // $pesoinicial_observacion = $row_balanza["pesoinicial_observacion"];
          $peso_final = $row_balanza["peso_final"];
          $peso_final = str_replace(',', '', $peso_final);
          $pesofinal_fechahora = $row_balanza["peso_final_fecha"];
          // $pesofinal_observacion = $row_balanza["pesofinal_observacion"];

          // // Genera el Código QR
          //   $url = $url_lims.'files/tickets/'.$cod_lote.((strlen($lote_ticket_orden) == 0) ? '' : ' - PARTE '.$lote_ticket_orden).'.pdf';
          //   $dir = 'files/tickets/qr/';

          //   $file_name = $dir.$cod_lote.((strlen($lote_ticket_orden) == 0) ? '' : ' - PARTE '.$lote_ticket_orden).'.png';
            ...
            $html = '<!DOCTYPE html>
                      <html lang="es">
                        ...
                        <body style="width: 100%; padding: 0px; text-align: center;">
                          <div style="width: 100%; margin-left: 0px; margin-top: 50px margin-right: 0px;">
          ...
          // GUARDAR PDF EN RUTA
          // ===============================
          $nombre_pdf = $cod_lote.((strlen($lote_ticket_orden) == 0) ? '' : ' - PARTE '.$lote_ticket_orden).'.pdf';
          $ruta_pdf   = "files/tickets/" . $nombre_pdf;

          // Obtener contenido del PDF
          $pdf_output = $document->output();

          // Guardar archivo
          file_put_contents($ruta_pdf, $pdf_output);

          // Mensaje:
            echo $Id.'. Archivo generado: '.$nombre_pdf;
            echo "<br>";
        }
      }
    }
